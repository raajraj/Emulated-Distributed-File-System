var BASE_URL = "http://127.0.0.1:5432/firebase";
...
function searchDeathPerCountry(resultDiv, payload) {
  ...
  axios.get(BASE_URL + '/countrydeathcount', {
    ...
    .then(function (response) {
      ...
      var data = response.data;
      var reducer = JSON.parse(data["reducer"]);

      if(reducer["Country"].hasOwnProperty(0)) {
        var result = "Total Deaths in <span class='text-uppercase fw-bold'>" + reducer["Country"][0] + "</span> - Cumulative: <span class='text-uppercase fw-bold'>" + reducer["Cumulative total cases"][0] + "</br></span> Number of Deaths: <span class='text-uppercase fw-bold'>" + reducer["No. of deaths"][0] + "</span>";
      } else {
        var result = "<p class='text-center'>No deaths occurred.</p>";
      }
...
function searchNoOfCasesByCountry(resultDiv, payload) {
  ...
  .then(function (response) {
    ...
    if(reducer["Country"].hasOwnProperty(0)) {
      var reducerHTML =  "<table class='table table-stripped'><thead><th>Country</th><th>No. of Deaths</th><tbody>";
      var reducerData = JSON.parse(data["reducer"]);

      var countries = Object.values(reducerData["Country"]);
      var noOfDeaths = Object.values(reducerData["No. of deaths"]);

      for(var j = 0; j < countries.length; j++) {
        reducerHTML += "<tr><td>" + countries[j] + "</td><td>" + noOfDeaths[j] + "</td></tr>";
      }
      reducerHTML += "</tbody></table>";
    } else {
      var reducerHTML = "<p class='text-center'>No cases found.</p>";
    }

    resultDiv.empty();
    resultDiv.append(reducerHTML);
  })
  .catch(function (error) {
    console.log(error);
  });
  ...
  axios.get(BASE_URL + '/analysisdeathpercountry')
    .then(function (response) {
      ...
      for (let i = 0; i < datasets.length; i++) {
        ...
        datasetData = JSON.parse(reducerData[dataset]);

        datasetHTML = "<h5 class='text-uppercase'>" + dataset + "</h5><table class='table table-stripped'><thead><th>Country</th><th>No. of Deaths</th><tbody>";

        var countries = Object.values(datasetData["Country"]);
        var noOfDeaths = Object.values(datasetData["No. of deaths"]);

        for(var j = 0; j < countries.length; j++) {
          datasetHTML += "<tr><td>" + countries[j] + "</td><td>" + noOfDeaths[j] + "</td></tr>";
        }

        datasetHTML += "</tbody></table>";
        reducerHTML += datasetHTML;
      }
